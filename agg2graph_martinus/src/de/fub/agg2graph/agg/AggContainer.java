...
public class AggContainer {
	...
	protected IAggregationStrategy aggregationStrategy;
	...
	public static AggContainer createContainer(File sourceFolder,
		...
		if (aggStrategy != null) {
			aggStrategy.setAggContainer(agg);
			agg.aggregationStrategy = aggStrategy;
		}
		if (cachingStrategy != null) {
			cachingStrategy.clear();
			cachingStrategy.setAggContainer(agg);
			agg.cachingStrategy = cachingStrategy;
		}
	...
	public void addConnection(AggConnection conn) {
		...
		cachingStrategy.addConnection(conn);
	}

	public void save() {
		cachingStrategy.save();
	}

	public void removeConnection(AggConnection conn) {
		cachingStrategy.removeConnection(conn);
	}
	...
	public void deleteNode(AggNode node) {
		...
		cachingStrategy.removeNode(node);
	}
	...
	public void extractNode(AggNode node) {
			...
			for (AggConnection outConn : new ArrayList<AggConnection>(
				...
				if (!inConn.getFrom().equals(outConn.getTo())) {
					if (inConn.getFrom().getConnectionTo(outConn.getTo()) == null) {
						cachingStrategy.addConnection(aggregationStrategy
		...
		// remove old things
		for (AggConnection inConn : new ArrayList<AggConnection>(node.getIn())) {
			cachingStrategy.removeConnection(inConn);
		}
		for (AggConnection outConn : new ArrayList<AggConnection>(node.getOut())) {
			cachingStrategy.removeConnection(outConn);
		}
		cachingStrategy.removeNode(node);
	}
	...
	private String toStringHelper(boolean onlyVisible) {
		...
		if (cachingStrategy.getNodeCount() > 100) {
			sb.append(String.format("%d active nodes\n",
					cachingStrategy.getNodeCount()));
		} else {
			for (AggNode node : cachingStrategy.clipRegion(TileManager.WORLD)) {
				if (!node.isVisible()) {
					continue;
				}
	...
	public void mergeNodes(AggNode mergeSource, AggNode mergeTarget) {
		...
		// remove source
		cachingStrategy.removeNode(mergeSource);
	}
	...
	private AggConnection findConn(AggNode from, AggNode to) {
		if (from.isShallow()) {
			AggNode fromAggNode;
			try {
				fromAggNode = ((DefaultCachingStrategy) cachingStrategy)
			...
			try {
				AggNode toAggNode = ((DefaultCachingStrategy) cachingStrategy)
