...
public class AttractionValueWithMatlab {
	public static Map<String, Double> value = new HashMap<String, Double>();
	
	String syms = "syms M N s1 s2 x y k F1 F2";
	String f1 = "F1= (2^(1/2)*M*N*y)/(pi^(1/2)*exp(y^2/(2*s1^2 + 2*s2^2))*(s1^2 + s2^2)^(1/2)*(2*s1^2 + 2*s2^2))";
	String f2 = "F2= k*(x-y)";
	String f1Subs = "F1Val= subs(F1, {M, N, s1, s2}, {";	//1, 5, 5, 5})
	String f2Subs = "F2Val= subs(F2, {k, x}, {";				//0.005, 1})
	String f1V;
	String f2V;
	private double y = -1;
	MatlabProxy proxy;
	
	public AttractionValueWithMatlab(MatlabProxy proxy) {
		this.proxy = proxy;
	}	
	
	public double getYValue(int M, int N, int s1, int s2, double k, double x) {
		String f1 = "(" + Math.sqrt(2)*M*N  + "*y)/(" + Math.sqrt(Math.PI) + "*exp(y^2/" + (2*Math.pow(s1, 2) + 2*Math.pow(s2, 2)) + ")*" + Math.sqrt((Math.pow(s1, 2) + Math.pow(s2, 2)))*(2*Math.pow(s1, 2) + 2*Math.pow(s2, 2)) + ")";
		String f2 = k + "*(" + x + "-y)";

		try {
			this.proxy.setVariable("y", -1);
			this.proxy.eval("y = solve('"+ f1 + "=" + f2 +"')");
			this.y = ((double[]) proxy.getVariable("y"))[0];		
