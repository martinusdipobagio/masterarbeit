...
//					i++;
				...
				if (isMatch) {
					System.out.println("GPS Point : " + segment.get(i).getID());
...
//				System.out.println(match.getGpsPoints());
	...
	private List<List<AggNode>> filterPaths(List<List<AggNode>> paths) {
		boolean found = false;
		Iterator<List<AggNode>> path = paths.iterator();
		while(path.hasNext()) {
			Iterator<AggNode> p = path.next().iterator();
			while(p.hasNext()) {
				if(found) {
					while(p.hasNext()) {
						p.next();
