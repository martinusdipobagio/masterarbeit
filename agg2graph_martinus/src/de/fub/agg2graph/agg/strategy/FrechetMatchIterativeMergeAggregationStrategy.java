...
public class FrechetMatchIterativeMergeAggregationStrategy extends
	...
	public FrechetMatchIterativeMergeAggregationStrategy() {
		TraceDistanceFactory.setClass(FreeSpaceMatch.class);
		traceDistance = TraceDistanceFactory.getObject();
		MergeHandlerFactory.setClass(IterativeClosestPointMerge.class);
	...
	public void aggregate(GPSSegment segment) {
		...
		for(Pair<List<AggConnection>, List<GPSEdge>> r : result) {
			if(mergeHandler == null) {
				...
				TreeAggMap copyMap = (TreeAggMap) ((FreeSpaceMatch)traceDistance).map;
				((IterativeClosestPointMerge)mergeHandler).map = copyMap;
			}
...
//			System.out.println(second.size());
			...
			addToMergeHandler(first, second, maxInitDistance);
			finishMatch();
		}
