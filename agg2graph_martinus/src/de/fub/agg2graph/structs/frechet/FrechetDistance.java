...
public class FrechetDistance {
	...
	/** Pair.first is point in Q Pair.second is point in P */
	 ...
	 */
	public HashMap<Integer, TreeSet<ILocation>> fromAgg = new HashMap<>();
	public HashMap<Integer, TreeSet<ILocation>> fromTrace = new HashMap<>();
	...
	private void addP(int cellIndexi, ILocation trace, ILocation other) {
		...
		if (cellIndexi != -1) {
			if (!fromAgg.containsKey(cellIndexi)
					|| fromAgg.get(cellIndexi) == null) {
				fromAgg.put(cellIndexi, new TreeSet<ILocation>());
			}
			fromAgg.get(cellIndexi).add(other);
		}
	}
	...
	private void addQ(int cellIndexj, ILocation agg, ILocation other) {
		...
		if (cellIndexj != -1) {
			if (!fromTrace.containsKey(cellIndexj)
					|| fromTrace.get(cellIndexj) == null) {
				fromTrace.put(cellIndexj, new TreeSet<ILocation>());
			}
			fromTrace.get(cellIndexj).add(other);
		}
	}
