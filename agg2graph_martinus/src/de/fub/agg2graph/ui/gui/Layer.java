...
public class Layer implements Hideable {
	...
	private void drawText(Graphics2D g2, ILocation location1,
			ILocation location2, RenderingOptions ro) {
		if (location1 == null || location2 == null
				|| !location1.isRelevant()) {
			return;
		}
		...
		int k = location1.getK();
		
		// get middle point
		java.awt.Point pMitte = new java.awt.Point((p1.x / 2 + p2.x / 2),
				(p1.y / 2 + p2.y / 2));
		drawObjects.add(new Text(new XYPoint(pMitte.x, pMitte.y, k), ro,
				("" + k)));
	}
	...
	private void drawLine(Graphics2D g2, ILocation location1,
			...
			boolean directed) {
		if (location1 == null || location2 == null
				|| ro.renderingType == RenderingType.POINTS || ro.renderingType == RenderingType.TEXT || !location1.isRelevant()) {
			return;
		}
	...
	private void drawPoint(Graphics2D g2, ILocation location,
			RenderingOptions ro) {
		if (location == null || ro.renderingType == RenderingType.LINES || ro.renderingType == RenderingType.TEXT
				|| !location.isRelevant()) {
			return;
		}
	...
	public boolean repaint(boolean force) {
		...
		for (Object loopObject : loopCopyObjects) {
			...
			if (ro.renderingType == RenderingType.NONE) {
				continue;
			}
				...
				if (visibleArea != null) {
					...
					if (nodes != null) {
						...
						if (layerManager.renderWeight) {
							...
							roVisible.color = Color.BLACK;
							roHidden = ro.getCopy();
							roHidden.color = new Color(95, 95, 95);
						}
			...
			if (object instanceof Set<?>) { // Set<Intersection>
				...
				intersectionRo.strokeBaseWidthFactor = layerManager.getSize().width / 50;
				intersectionRo.strokeBaseWidthFactor = 7 + 17 / 6 * (layerManager
...
//TODO					
		...
		for (DrawObject drawObject : internalLoopDrawObjects) {
			...
			Color lineColor = new Color((float) (ro.color.getRed() / 255.0),
					(float) (ro.color.getGreen() / 255.0),
					(float) (ro.color.getBlue() / 255.0), (float) ro.opacity);
			g2.setColor(lineColor);

			if (drawObject instanceof Point) {
				Point p = (Point) drawObject;
				// draw point
				if (ro.renderingType == RenderingType.POINTS) {
					float width = ro.strokeBaseWidthFactor
							* RenderingOptions.basicStroke.getLineWidth();
				...
				if (p.at.getID() == null
						|| ro.labelRenderingType == LabelRenderingType.NEVER) {
					continue;
				}
				if (ro.labelRenderingType == LabelRenderingType.INTELLIGENT
				...
				if (ro.renderingType == RenderingType.LINES) {
					continue;
				}
				if (ro.renderingType == RenderingType.INTELLIGENT_ALL
			...
			//TODO New!!!
			} else if (drawObject instanceof Text) {
				Text t = (Text) drawObject;
				// draw text
//				if (ro.renderingType == RenderingType.TEXT) {
					float width = ro.strokeBaseWidthFactor
							* RenderingOptions.basicStroke.getLineWidth();
					g2.drawString(t.text, (int) (t.at.getX() - width / 2),
							(int) (t.at.getY() - width / 2));
//				}
				// draw label
				// do not draw label if not fully opaque
				if (ro.opacity < 1) {
...
//								.getHeight() * 0.075));
	...
	public class Text implements DrawObject {

		public ILocation at;
		public RenderingOptions renderingOptions;
		public String text;

		public Text(ILocation at, RenderingOptions ro, String text) {
			this.at = at;
			this.renderingOptions = ro;
			this.text = text;
		}
