...
public class MainRenderingPanel extends OsmPanel implements IRenderingPanel {
		...
		labels.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				for (Layer layer : getUi().getLayerManager().getLayers()) {
					if (showingLabels) {
						layer.getOptions().labelRenderingType = LabelRenderingType.ALWAYS;
					} else {
						layer.getOptions().labelRenderingType = LabelRenderingType.NEVER;
					}
				}
				outerThis.repaint();
			}
	...
	private void highlightLayer(int wheelRotation) {
		if (savedZIndex != Integer.MAX_VALUE) {
			layers.get(highlightIndex).getOptions().zIndex = savedZIndex;
		}
		highlightIndex = (highlightIndex + wheelRotation + layers.size())
				% layers.size();
		savedZIndex = layers.get(highlightIndex).getOptions().zIndex;
		layers.get(highlightIndex).getOptions().zIndex = 999;

		if (transparentHighlighting) {
			for (Layer layer : layers) {
				layer.getOptions().opacity = 0.3;
			}
			layers.get(highlightIndex).getOptions().opacity = 1;
		}
		getUi().getLayerManager().repaintAllLayers();
	}
